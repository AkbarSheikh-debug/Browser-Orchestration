  ...
  return (
    <div className="relative flex size-full items-center justify-center bg-black">
      <video
        ...
        controls
        className="size-full"
        // Hidden until ready so the loading/error note shows through instead of
        // an empty black frame.
        style={{ display: status === "ready" ? "block" : "none" }}
      />
      {status === "loading" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-xs text-[#5A6273]">
          <Loader2 className="size-4 animate-spin" />
          <span>Preparing recording…</span>
        </div>
      )}
      {status === "error" && (
        <div className="absolute inset-0 flex items-center justify-center p-3 text-center text-xs text-[#5A6273]">
